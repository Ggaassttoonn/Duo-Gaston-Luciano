...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        h1 { text-align: center; color: #2c3e50; border-bottom: 3px solid #3498db; padding-bottom: 10px; margin-bottom: 25px; font-size: 24px; }
        h2 { color: #e67e22; margin-top: 25px; margin-bottom: 10px; font-size: 18px; border-bottom: 1px solid #ddd; padding-bottom: 5px; }
        .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px; }
        .info-box { background: #fff; padding: 12px; border-left: 5px solid #3498db; border-radius: 4px; font-size: 14px; }
    ...
    <div class="container">
        ...
        <div class="info-grid">
            ...
            <div class="info-box">
                ...
                {{ $planificacion->fecha_estimada?->format('d/m/Y') }}
            </div>
            <div class="info-box">
                <strong>Fecha Presentación:</strong>
                {{ $planificacion->fecha_presentacion?->format('d/m/Y') }}
            </div>
            @if ($planificacion->fecha_desarrollada)
            <div class="info-box">
                <strong>Fecha Desarrollada:</strong>
                {{ $planificacion->fecha_desarrollada?->format('d/m/Y') }}
            </div>
            @endif
            <div class="info-box">
                <strong>Tipo:</strong>
        ...
        <div class="section-content">{{ $planificacion->contenidos_especificos }}</div>

        <h2>Actividades</h2>
        <div class="section-content">{{ $planificacion->actividades }}</div>

        @if ($planificacion->tareas)
        <h2>Tareas</h2>
        <div class="section-content">{{ $planificacion->tareas }}</div>
        @endif

        @if ($planificacion->estadosDiarios->isNotEmpty())
        <h2>Estados</h2>
        <div class="section-content">
            <ul>
                @foreach ($planificacion->estadosDiarios as $estado)
